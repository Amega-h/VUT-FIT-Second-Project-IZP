...
{
    assert(narr > 0);
    float mindis=cluster_distance(&carr[0],&carr[1]);
    for (int i=0;i<narr;i++)
    {
        for(int j=i+1;j<narr;j++)
        {
            if(cluster_distance(&carr[i],&carr[j])<mindis)
            {
                mindis=cluster_distance(&carr[i],&carr[j]);
                *c1=i;
                *c2=j;
            }
        }
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    int countobj=0;
    if(!fscanf(filik, "count=%d", &countobj))
    {
        fprintf(stderr, "Wrong file format\n");
        fclose(filik);
        return -1;
    }
    *arr = malloc(sizeof(struct cluster_t)*countobj);
    for (int i=0;i<countobj;i++)
    {
        struct obj_t objtemp;
        fscanf(filik,"%d %g %g",&objtemp.id,&objtemp.x,&objtemp.y);
        struct cluster_t temp;
        init_cluster(&temp,1);
        append_cluster(&temp,objtemp);
        (*arr)[i]=temp;
    }
    fclose(filik);
    return countobj;
...
int main(int argc, char *argv[])
{
    ...
    if (argc!=3)
    {
        ...
        return -1;
    }else if (argc == 3 && !CheckErrSymb(argv[2])) // checks if Argument has something that is not a number , if so , prints error message
        {
        ...
        } else 
        {
            int index1=0;
            int index2=0;
            countclusters=load_clusters(argv[1],&clusters);
            while (countclusters != atoi(argv[2]))
            {
